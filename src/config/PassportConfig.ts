...
const AuthErrors = {
  UserNotFound: (customMsg?: string) => createAuthError(customMsg || "User not found", 404),
  AlreadyRegistered: createAuthError("This email is already registered", 400),
  PasswordTooShort: createAuthError("The password must be at least 8 characters", 400),
  EmailNotPendingRegistration: createAuthError("Email not found for pending registration", 404),
  EmailNotValidated: createAuthError("User did not complete email validation", 400),
...
passport.use(
  "register",
  new LocalStrategy(strategyOptions, async (req:Request, username, password, done) => {
    try {
      const { name, surname, phoneNumber, address } = req.body;

      const userStored = await userService.getUserById(username);
      if (!userStored) {
        return done(null, false, AuthErrors.EmailNotPendingRegistration as any);
      }
      ...
      if (error instanceof Error && error.message === `there is no document with id: ${username}`) {
        return done(null, false, AuthErrors.EmailNotPendingRegistration as any);
      }
